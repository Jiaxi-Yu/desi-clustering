...
desipipe tasks -q data_dr3_bao  # check the list of tasks
desipipe spawn -q data_dr3_bao --spawn  # spawn the jobs
desipipe queues -q data_dr3_bao  # check the queue
Or directly if mode = 'interactive':
salloc -N 1 -C "gpu&hbm80g" -t 02:00:00 --gpus 4 --qos interactive --account desi_g
srun -n 4 python job_scripts/data_dr3_bao.py
"""
...
output, error = './slurm_outputs/data_dr3_bao/slurm-%j.out', './slurm_outputs/data_dr3_bao/slurm-%j.err'
kwargs = {}
...
def run_stats(version='data-dr3-daily-test', tracer='LRG', weight='default-FKP', zranges=None, stats_dir=Path(os.getenv('SCRATCH')) / 'measurements', stats=['mesh2_spectrum'], ibatch=None, project='bao/base', **kwargs):
    ...
    for region in ['NGC', 'SGC', 'N', 'NGCnoN', 'S', 'SGCnoDES'][2:]:
        ...
        particle2_correlation = {'battrs': battrs, 'jackknife': {'nsplits': 60}, 'split_randoms': (2., 20)}
        ...
        propose_recon_cellsizes = {'BGS': 7., 'LRG': 7., 'LGE': 7., 'ELG': 7., 'LRG+ELG': 7., 'LRG+LGE': 7., 'QSO': 10.}
        recon = {'mattrs': {'boxpad': 1.2, 'cellsize': propose_recon_cellsizes[tools.get_simple_tracer(tracer)], 'primes': primes, 'divisors': divisors}}
        options = dict(catalog=dict(version=version, tracer=tracer, zrange=zranges, region=region, weight=weight), recon=recon, mesh2_spectrum={}, window_mesh2_spectrum={}, particle2_correlation=particle2_correlation, recon_particle2_correlation=particle2_correlation, window_mesh3_spectrum={'ibatch': ibatch} if isinstance(ibatch, tuple) else {'computed_batches': ibatch})
        # Fill in missing options with default/fiducial values from tools
        options = fill_fiducial_options(options)
        # Compute all requested statistics
        compute_stats_from_options(stats, get_stats_fn=get_stats_fn, cache=cache, analysis='full_shape_protected', **options)


def postprocess_stats(version='data-dr3-daily-test', tracer='LRG', weight='default-FKP', stats_dir=Path(os.getenv('SCRATCH')) / 'measurements', project='bao/base', postprocess=['combine_regions'], **kwargs):
...
if __name__ == '__main__':
    ...
    for tracer in ['BGS', 'LRG', 'ELG', 'QSO'][2:3]:
        ...
        if stats:
            get_run_stats()(version=version, tracer=tracer, zranges=zranges, stats_dir=stats_dir, project=project, stats=stats)
        # Execute post-processing if requested (combine regions, etc.)
        if postprocess:
            postprocess_stats(version=version, tracer=tracer, zranges=zranges, stats_dir=stats_dir, project=project, postprocess=postprocess)