...
desipipe tasks -q data_dr2_bao  # check the list of tasks
desipipe spawn -q data_dr2_bao --spawn  # spawn the jobs
desipipe queues -q data_dr2_bao  # check the queue
Or directly if mode = 'interactive':
salloc -N 1 -C "gpu&hbm80g" -t 02:00:00 --gpus 4 --qos interactive --account desi_g
srun -n 4 python job_scripts/data_dr2_bao.py
"""
...
output, error = './slurm_outputs/data_dr2_bao/slurm-%j.out', './slurm_outputs/data_dr2_bao/slurm-%j.err'
kwargs = {}
...
def run_stats(version='data-dr2-v1.1', tracer='LRG', weight='default-FKP', zranges=None, stats_dir=Path(os.getenv('SCRATCH')) / 'measurements', stats=['mesh2_spectrum'], ibatch=None, project='bao/base', **kwargs):
...
def postprocess_stats(version='data-dr2-v1.1', tracer='LRG', weight='default-FKP', stats_dir=Path(os.getenv('SCRATCH')) / 'measurements', project='bao/base', postprocess=['combine_regions'], **kwargs):
...
if __name__ == '__main__':
    ...
    #stats_dir = Path(os.environ['SCRATCH'])
