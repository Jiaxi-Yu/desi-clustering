...
basedir        = '/global/cfs/cdirs/desi/science/cpe/cmbdata/momento/'
ncm_dir        = basedir
tau_dir        = basedir
reijo_dir      = basedir
planck2018_dir = basedir
sroll20_dir    = basedir
...
tmp100 = hp.read_map(smooth_mean_100, field=(1,2))
tmp143 = hp.read_map(smooth_mean_143, field=(1,2))
...
if removefidCMBfrommeans:
    cmb100i,cmb100q,cmb100u=hp.read_map('RC4/cmb_fid_100_lr_no_md.fits',field=(0,1,2))
    cmb143i,cmb143q,cmb143u=hp.read_map('RC4/cmb_fid_143_lr_no_md.fits',field=(0,1,2))
...
if removefidskyfrommeans:
    ...
    cmb100i,cmb100q,cmb100u=hp.read_map(inname,field=(0,1,2))
    inname='RC4/inp143_lr_nmd.fits'
    cmb143i,cmb143q,cmb143u=hp.read_map(inname,field=(0,1,2))
    inname='RC4/inp353_lr_nmd.fits'
    cmb353i,cmb353q,cmb353u=hp.read_map(inname,field=(0,1,2))
...
if usefitsdata:
    ...
    cmb30i,cmb30q,cmb30u=hp.read_map(inname,field=(0,1,2))
    inname='RC4/100GHz_RC4_lr_nmd.fits'
    cmb100i,cmb100q,cmb100u=hp.read_map(inname,field=(0,1,2))
    inname='RC4/143GHz_RC4_lr_nmd.fits'
    cmb143i,cmb143q,cmb143u=hp.read_map(inname,field=(0,1,2))
    inname='RC4/353GHz_RC4_lr_nmd.fits'
    cmb353i1,cmb353q1,cmb353u1=hp.read_map(inname,field=(0,1,2))
    inname='RC4/353GHz_RC4_lr_nmd.fits'
    cmb353i2,cmb353q2,cmb353u2=hp.read_map(inname,field=(0,1,2))
...
if usegeneralfitsdata_roger:
    ...
    print('',flush=True)
    cmb30i1,cmb30q1,cmb30u1=hp.read_map(inname30_1,field=(0,1,2))
    cmb30i2,cmb30q2,cmb30u2=hp.read_map(inname30_2,field=(0,1,2))
    cmb100i,cmb100q,cmb100u=hp.read_map(inname100,field=(0,1,2))
    cmb143i,cmb143q,cmb143u=hp.read_map(inname143,field=(0,1,2))
    cmb353i1,cmb353q1,cmb353u1=hp.read_map(inname353_1,field=(0,1,2))
    cmb353i2,cmb353q2,cmb353u2=hp.read_map(inname353_2,field=(0,1,2))
...
if usegeneralfitsdata:
    ...
    print(inname143,flush=True)
    print('',flush=True)
    cmb30i,cmb30q,cmb30u=hp.read_map(inname30,field=(0,1,2))
    cmb100i,cmb100q,cmb100u=hp.read_map(inname100,field=(0,1,2))
    cmb143i,cmb143q,cmb143u=hp.read_map(inname143,field=(0,1,2))
    cmb353i1,cmb353q1,cmb353u1=hp.read_map(inname353_1,field=(0,1,2))
    cmb353i2,cmb353q2,cmb353u2=hp.read_map(inname353_2,field=(0,1,2))
...
def reijo(ell, nside):
    if (ell<=nside): wl = 1.
    if (nside < ell <= 3.*nside): wl = 0.5*(1+ cos(pi * (ell-nside)/(2*nside)))
    if (ell > 3.*nside): wl = 0.
    return wl

vreijo = vectorize(reijo)
if smooth_reijo:
    b_l    = vreijo(arange(0, 3*nside), nside)
elif smooth_reijo_pixwin:
    b_l    = vreijo(arange(0, 3*nside), nside)*hp.pixwin(nside)
...
#CHECK: Should this have the beam in or not?qqqq
def getclsmulti_camb(cls_theory_camb, wantedcols):
    #ATTENTION: cl_theory_camb start at ell=0
    ells=arange(lmin, lmax+1.,1.)
    cls_theory_TT = cls_theory_camb['tt'][lmin:lmax+1]
    cls_theory_TE = cls_theory_camb['te'][lmin:lmax+1]
    cls_theory_EE = cls_theory_camb['ee'][lmin:lmax+1]
    cls_theory_BB = cls_theory_camb['bb'][lmin:lmax+1]
    cls_theory_all = vstack((ells,cls_theory_TT,cls_theory_EE,cls_theory_BB,cls_theory_TE)).T
    ncols=len(wantedcols)
    cls_tmp=zeros((lmax+1-lmin)*ncols)
    wcol=0
    for col in wantedcols:
        cls_tmp[wcol::ncols]=2.0*math.pi*cls_theory_all[:,col]/ells/(ells+1.)*b_l[lmin:lmax+1]**2
        wcol+=1
    return cls_tmp
...
def makecllong_camb(cls_theory_camb):
    cls_theory_TT = cls_theory_camb['tt'][lmin:lmax+1]
    cls_theory_TE = cls_theory_camb['te'][lmin:lmax+1]
    cls_theory_EE = cls_theory_camb['ee'][lmin:lmax+1]
    cls_theory_ET = cls_theory_TE
    outlen=2*((lmax+1)**2-lmin**2)
    cldiag=zeros((outlen,outlen))
    for l in range(lmin,lmax+1):
        for m in range(0,2*l+1):
            outpos=2*(l**2+m-lmin**2)
            cldiag[outpos,outpos]    = 2.0*math.pi*cls_theory_TT[l-lmin]/l/(l+1)*b_l[l]**2
            cldiag[outpos,outpos+1]  = 2.0*math.pi*cls_theory_TE[l-lmin]/l/(l+1)*b_l[l]**2
            cldiag[outpos+1,outpos]  = 2.0*math.pi*cls_theory_ET[l-lmin]/l/(l+1)*b_l[l]**2
            cldiag[outpos+1,outpos+1]= 2.0*math.pi*cls_theory_EE[l-lmin]/l/(l+1)*b_l[l]**2
    return cldiag
...
def s_romb_parallel(cls):
    ...
    if False:
        ...
        result = [gradterm(*tmp) for tmp in workargs]
    grads[1:]=result
    # delete following lines to revert to romberg integration
    if int_method == 'romberg':
        return scipy.integrate.romb(grads,.25,show=False)
...
def s_romb_part_parallel(cls):
    ...
    if False:
        ...
        result = [gradterm_part(*tmp) for tmp in workargs]
    grads[1:]=result
    # delete following lines to revert to romberg integration
    if int_method == 'romberg':
        return scipy.integrate.romb(grads,.25,show=False)
...
def s_romb_diff_part_parallel(cls1,cls2):
    ...
    if False:
        ...
        result = [gradtermdiff_part(*tmp) for tmp in workargs]
    grads=result
    # delete following lines to revert to romberg integration
    if int_method == 'romberg':
        return scipy.integrate.romb(grads,.25,show=False)
...
plt.figure(1)
plt.clf()
plt.suptitle(r'{}{}x{}{} GHz full Power Spectra ({})'.format(freq1,split1,freq2,split2,data_set))

plt.subplot(2,2,1)
plt.axhline(0,color='r')
plt.plot(ells,dl_theory_ee,'k--')
plt.errorbar(ells,dl_12_fid_ee,yerr=dl_err_fid_ee,fmt='b.')
plt.xlim(0,lmax+1)
plt.ylim(0,2000)
plt.title(r'$D_l^\mathrm{TT}$')

plt.subplot(2,2,2)
plt.plot(ells,dl_theory_eb, 'k--')
plt.errorbar(ells,dl_12_fid_eb,yerr=dl_err_fid_eb,fmt='b.')
plt.xlim(0,lmax+1)
plt.ylim(-5,10)
plt.title(r'$D_l^\mathrm{TE}$')

plt.subplot(2,2,3)
plt.plot(ells,dl_theory_be,'k--')
plt.errorbar(ells,dl_12_fid_be,yerr=dl_err_fid_be,fmt='b.')
plt.xlim(0,lmax+1)
plt.ylim(-5,10)
plt.title(r'$D_l^\mathrm{ET}$')

plt.subplot(2,2,4)
plt.plot(ells,dl_theory_bb, 'k--')
plt.errorbar(ells,dl_12_fid_bb,yerr=dl_err_fid_bb,fmt='b.')
try:
    savetxt(basedir+'qml/ps_{}_{}{}x{}{}_ell_{}-{}_tau_0.{:03d}-0.{:03d}{}{}.txt'.format(data_set,freq1,split1,freq2,split2,lminlike,lmaxlike,imin,imax, linear_approx,pol_approx), c_[ells,dl_12_fid_ee,dl_err_fid_ee,dl_12_fid_eb,dl_err_fid_eb,dl_12_fid_be,dl_err_fid_be,dl_12_fid_bb,dl_err_fid_bb])
except (FileNotFoundError, IOError):
    pass
plt.xlim(0,lmax+1)
plt.ylim(pymin,pymax)
plt.title(r'$D_l^\mathrm{EE}$')

try:
    plt.savefig(basedir+'plots/ps_{}_{}{}x{}{}_ell_{}-{}_tau_0.{:03d}-0.{:03d}{}{}.pdf'.format(data_set,freq1,split1,freq2,split2,lminlike,lmaxlike,imin,imax,linear_approx,pol_approx))
except (FileNotFoundError, IOError):
    pass
if show_plot:
    plt.show()
...
class TTTEEE_SROLL20(Likelihood):
    def initialize(self):
        """
    ...
    def get_requirements(self):
        ...
        """
        return {'Cl': {'tt': 60, 'te': 60, 'ee': 60}, 'tau': None}
    ...
    def logp(self, **params_values):
        ...
        calculate log-likelihood using momento and input theory D_ell for TT,TE,EE
        """
        cls_theory    = self.provider.get_Cl(ell_factor=True, units="muK2")
        #print(cls_theory['tt'].size, cls_theory['te'].size, cls_theory['ee'].size, cls_theory['bb'].size)
        cobaya_mll = 0.
        fidcovar=dot(cipart,dot(cov_12_pq_fid,cipart_t))
        cllong=makecllong_camb(cls_theory)
        cov_11=make_m_slow(f_11,cllong, f_11, yn_11)
        cov_12=make_m_slow(f_11,cllong, f_22, yn_12)
        cov_21=make_m_slow(f_22,cllong, f_11, yn_21)
        cov_22=make_m_slow(f_22,cllong, f_22, yn_22)
        #print('shapes cov:', cov_11.shape, cov_12.shape, cov_21.shape, cov_22.shape)
        cov_12_pq=cov_12_34(cov_21,cov_21,cov_22,cov_11)    
        #print('shape cov :', cov_12_pq.shape)
        m=sum_mat(cov_12)
        #print('m         :', m)
        emm=dot(cipart,m)
        #print('emm       :', emm)
        cls_momento=getclsmulti_camb(cls_theory,(1,4,2))
        #print('cls       :', cls_momento)
        cobaya_mll=s_romb_part_parallel(cls_momento)
        cobaya_mll=-1.0*cobaya_mll
        tau_theory = self.provider.get_param("tau")
        #print ('-----------------------------')
        #print ('tau theory:    ', tau_theory)
        #print ('log-likelihood ', cobaya_mll)
