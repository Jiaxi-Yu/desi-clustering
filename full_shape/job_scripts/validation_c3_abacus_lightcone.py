...
DEFAULT_STATS_DIR = Path('/dvs_ro/cfs/cdirs/desicollab/science/cai/desi-clustering/dr2/summary_statistics')
...
def _build_likelihoods_options(stats, tracers, zranges, version, stats_dir, project, theory_model,
                               syst_templates=tuple(), emulator=True, prior_basis='physical_aap'):
    _validate_theory_model(stats, theory_model)
    likelihoods = []
    for tracer, zrange in zip(tracers, zranges, strict=True):
...
def _build_run_options(stats, tracers, zranges, version, stats_dir, project, theory_model,
...
def run_fit(actions=('profile',), template='direct', version='abacus-2ndgen-dr2-complete',
            ...
            stats=['mesh2_spectrum'], tracers=None, zranges=None, theory_model='folpsD',
    ...
    get_fits_fn = functools.partial(tools.get_fits_fn, fits_dir=fits_dir, level={'catalog': 3})
...
if __name__ == '__main__':
    ...
    base_fits_dir = Path(args.fits_dir)
    dataset = 'abacus-hf-lc-dr2-v1.9'
    fits_dir = base_fits_dir / dataset
    ...
    for zrange in [(0.8, 2.1), (0.8, 1.6), (1.6, 2.1)]:
        run_fit(actions=args.todo, version=dataset, stats_dir=stats_dir, project='full_shape/lightcone_validation',
                fits_dir=fits_dir, cache_dir=cache_dir, stats=args.stats, tracers=['QSO'], zranges=[zrange], theory_model=args.theory_model,
